...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function addServiceField(
    serviceData = { name: "", description: "", icon: "fas fa-cog" }
  ) {
    const serviceId = Date.now();
    const serviceHTML = `
      <div class="service-item" data-id="${serviceId}">
        <input type="text" name="serviceName" placeholder="Nombre del servicio" value="${
          serviceData.name
        }" required>
        <input type="text" name="serviceDescription" placeholder="Descripción" value="${
          serviceData.description
        }">
        <select name="serviceIcon" class="service-icon-select">
          <option value="fas fa-cog" ${
            serviceData.icon === "fas fa-cog" ? "selected" : ""
          }>Configuración</option>
          <option value="fas fa-paint-brush" ${
            serviceData.icon === "fas fa-paint-brush" ? "selected" : ""
          }>Diseño</option>
          <option value="fas fa-code" ${
            serviceData.icon === "fas fa-code" ? "selected" : ""
          }>Desarrollo</option>
          <option value="fas fa-shopping-cart" ${
            serviceData.icon === "fas fa-shopping-cart" ? "selected" : ""
          }>Ventas</option>
          <option value="fas fa-wrench" ${
            serviceData.icon === "fas fa-wrench" ? "selected" : ""
          }>Reparación</option>
          <option value="fas fa-heart" ${
            serviceData.icon === "fas fa-heart" ? "selected" : ""
          }>Salud</option>
          <option value="fas fa-utensils" ${
            serviceData.icon === "fas fa-utensils" ? "selected" : ""
          }>Comida</option>
        </select>
  ...
  window.removeService = function (id) {
    const serviceToRemove = document.querySelector(
      `.service-item[data-id="${id}"]`
    );
    if (serviceToRemove) {
  ...
  function initDropzones() {
    ...
    const imagesInput = document.querySelector(
      '#imagesDropzone input[type="file"]'
    );
    ...
    daySelect.addEventListener("change", async function () {
      ...
      if (isDayAlreadySelected(selectedDay)) {
        ...
        if (action === "cancel") {
          ...
          if (
            container.querySelectorAll(".different-hours-item").length === 0
          ) {
            container.style.display = "none";
            document.getElementById("addDifferentHoursBtn").style.display =
              "block";
          }
        }
      }
  ...
  function handleLogoUpload(file) {
    ...
    if (!validTypes.includes(file.type)) {
      showError(
        "Formato de archivo no válido para el logo. Use JPG, PNG o SVG"
      );
      return;
    }
  ...
  function handleHeroUpload(file) {
    ...
    if (!validTypes.includes(file.type)) {
      showError(
        "Formato de archivo no válido para la imagen principal. Use JPG o PNG"
      );
      return;
    }
  ...
  function handleAboutUpload(file) {
    ...
    if (!validTypes.includes(file.type)) {
      showError(
        "Formato de archivo no válido para la imagen sobre nosotros. Use JPG o PNG"
      );
      return;
    }
  ...
  function handleImagesUpload(files) {
    const imagesPreview = document.getElementById("imagesPreview");
    const currentImages =
      imagesPreview.querySelectorAll(".uploaded-image").length;
  ...
  window.removeGalleryImage = function (id, fileName) {
    const imageToRemove = document.querySelector(
      `.uploaded-image[data-id="${id}"]`
    );
    if (imageToRemove) {
  ...
  function getBusinessHoursSummary() {
    const days = Array.from(
      document.querySelectorAll('input[name="businessDays"]:checked')
    ).map((el) => el.value);
  ...
  function fillFormWithData(data) {
    ...
    if (data.businessSlogan)
      document.getElementById("businessSlogan").value = data.businessSlogan;
    if (data.businessDescription)
      document.getElementById("businessDescription").value =
        data.businessDescription;
    if (data.businessShortDescription)
      document.getElementById("businessShortDescription").value =
    ...
    if (data.servicesDescription)
      document.getElementById("servicesDescription").value =
        data.servicesDescription;
    if (data.yearsExperience)
      document.getElementById("yearsExperience").value = data.yearsExperience;
    if (data.happyClients)
      document.getElementById("happyClients").value = data.happyClients;
    if (data.businessAddress)
      document.getElementById("businessAddress").value = data.businessAddress;
    if (data.googleMaps)
      document.getElementById("googleMaps").value = data.googleMaps;
  ...
  function addSocialFieldWithValue(platform, value) {
    ...
    document
      ...
      .addEventListener("click", function () {
        ...
        const socialOption = document.querySelector(
          `.social-option[data-social="${platform}"]`
        );
        if (socialOption) {
  ...
  function collectFormData() {
    const formData = {
      businessInfo: {
        ...
        shortDescription: document.getElementById("businessShortDescription")
          .value,
        phone: iti.getNumber(),
        email: document.getElementById("contactEmail").value,
        aboutUs: document.getElementById("aboutUs").value,
        servicesDescription: document.getElementById("servicesDescription")
          .value,
    ...
    document.querySelectorAll(".service-item").forEach((item) => {
      formData.services.push({
        ...
        description: item.querySelector("input[name='serviceDescription']")
          .value,
  ...
  async function uploadImageToCloudinary(file) {
    try {
      ...
      if (result.secure_url) {
        ...
        throw new Error(
          `Error de Cloudinary: ${result.error?.message || "Error desconocido"}`
        );
      }
    } catch (error) {
      ...
      if (
        ...
        throw new Error(
          "Error de conexión. Verifica tu conexión a internet y que Cloudinary esté disponible."
        );
      }
      throw error;
    }
